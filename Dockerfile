...
# wget wrapper with retries — CDN sources rate-limit GitHub Actions IPs.
RUN printf '#!/bin/sh\nfor i in 1 2 3 4 5; do\n  wget -q --tries=3 --timeout=60 --waitretry=5 "$@" && exit 0\n  echo "wget attempt $i failed, retrying in 10s..." >&2\n  sleep 10\ndone\nexit 1\n' > /usr/local/bin/wget-retry && chmod +x /usr/local/bin/wget-retry

# NDK
RUN wget-retry -q https://dl.google.com/android/repository/android-ndk-r27c-linux.zip -O /tmp/ndk.zip \
...
RUN wget-retry -q https://github.com/PCRE2Project/pcre2/releases/download/pcre2-10.44/pcre2-10.44.tar.gz && tar xf pcre2-10.44.tar.gz && cd pcre2-10.44 && ./configure --host=aarch64-linux-android --prefix=${PREFIX} --enable-static --disable-shared CC="${CC}" && make -j$(nproc) install
RUN wget-retry -q https://github.com/libffi/libffi/releases/download/v3.4.6/libffi-3.4.6.tar.gz && tar xf libffi-3.4.6.tar.gz && cd libffi-3.4.6 && ./configure --host=aarch64-linux-android --prefix=${PREFIX} --enable-static --disable-shared CC="${CC}" && make -j$(nproc) install
...
RUN wget-retry -q https://download.gnome.org/sources/glib/2.82/glib-2.82.5.tar.xz&& tar xf glib-2.82.5.tar.xz && cd glib-2.82.5 && meson setup _build --cross-file /opt/cross-android-aarch64.ini --prefix ${PREFIX} --default-library static -Dselinux=disabled -Dlibmount=disabled && ninja -C _build install
RUN wget-retry -q https://cairographics.org/releases/pixman-0.44.2.tar.xz && tar xf pixman-0.44.2.tar.xz && cd pixman-0.44.2 && meson setup _build --cross-file /opt/cross-android-aarch64.ini --prefix ${PREFIX} --default-library static -Da64-neon=disabled && ninja -C _build install
# attr — download.savannah.gnu.org is unreachable from GH Actions.
# Android Bionic already provides xattr symbols in libc, so a stub is enough.
RUN printf '/* stub: Android Bionic provides xattr in libc */\n' > /tmp/attr_stub.c \
    && ${CC} --sysroot=${LLVM}/sysroot -target aarch64-linux-android26 -c /tmp/attr_stub.c -o /tmp/attr_stub.o \
    && ${AR} rcs ${PREFIX}/lib/libattr.a /tmp/attr_stub.o \
...
RUN wget-retry -q https://github.com/libusb/libusb/releases/download/v1.0.27/libusb-1.0.27.tar.bz2 \
...
RUN wget-retry -q https://download.qemu.org/${QEMU_DIR}.tar.xz && tar xf ${QEMU_DIR}.tar.xz
